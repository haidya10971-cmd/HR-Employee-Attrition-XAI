...
_HR_CONSULTANT_PROMPT = (
    PromptTemplate(
        input_variables=["context", "question"],  # type: ignore[arg-type]
        template=(
...
# ─────────────────────────────────────────────────────────────────────────────
# 💬 HR Chatbot — Interactive Q&A powered by Groq (always visible)
# ─────────────────────────────────────────────────────────────────────────────
st.divider()
st.subheader("💬 HR AI Chatbot")
st.caption(
    "Ask anything about HR policies, attrition risk factors, or employee retention strategies. "
...
# ── Chat input box ────────────────────────────────────────────────────────────
user_question = st.chat_input("Ask the HR AI Chatbot…")
    ...
    with st.chat_message("assistant"):
        ...
        if not groq_key:
            answer = (
                ...
                "Please enter your Groq API key in the **AI Consultant Settings** "
                "panel in the sidebar to activate the chatbot."
            )
        elif not _RAG_AVAILABLE:
            answer = (
                f"⚠️ **RAG libraries not installed.**\n\n"
                "Run:\n```\npip install langchain-core langchain-community "
                "langchain-groq langchain-chroma langchain-text-splitters "
                "chromadb pypdf fastembed\n```\n\n"
                f"Import error: `{_RAG_IMPORT_ERROR}`"
            )
        else:
            try:
                # Build a compact employee context if a prediction was made
                employee_ctx = ""
                if predict_btn:
                    try:
                        _r, _rp = _predict_employee(inputs)
                        employee_ctx = (
                            f"\n\nCURRENT EMPLOYEE LOADED IN THE DSS:\n"
                            f"  Risk Score : {_r:.1f}%  "
                            f"({'HIGH RISK' if _rp >= optimal_threshold else 'LOW RISK'})\n"
                            f"  Department : {inputs.get('Department', 'N/A')}\n"
                            f"  Job Role   : {inputs.get('JobRole', 'N/A')}\n"
                            f"  OverTime   : {inputs.get('OverTime', 'N/A')}\n"
                        )
                    except Exception:
                        employee_ctx = ""
                ...
                history_text = ""
                for h in st.session_state.chat_history[:-1]:  # exclude current message
                    role_label = "User" if h["role"] == "user" else "Assistant"
                    history_text += f"{role_label}: {h['content']}\n"

                system_prompt = (
                    "You are an expert AI HR Consultant specialising in employee retention, "
                    "attrition risk analysis, and HR policy interpretation. "
                    "You assist HR professionals by answering questions about:\n"
                    "  • Employee attrition risk factors and XAI explanations\n"
                    "  • HR policies and retention strategies\n"
                    "  • Best practices for employee engagement\n"
                    "  • Interpretation of model predictions\n\n"
                    "Be concise, professional, and actionable. "
                    "When relevant, reference HR policy articles and sections."
                    + employee_ctx
                )
                ...
                # Add prior turns for multi-turn memory
                ...
                answer = f"❌ **Chatbot error:** {chat_exc}"
